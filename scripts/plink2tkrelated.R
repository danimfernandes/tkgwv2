    ...
    for(line in seq(1:length(combos[,1]))) {
      ...
      if(file.size(paste0("comm",samp1,"_",samp2,"_SNPs")) > 0) {
        ...
        if(length(pairLoadNew$chr) >= ignoreThresh) {
          ...
          cat(paste0("\t # Estimated coefficient of relatedness Rxy for   ",samp1,"   ",samp2,"   ",round(relatednessR,3),"   (",line,"/",length(combos[,1]),")\n"))
        ...
        cat(paste0("\t # SKIPPING due to no overlapping SNPs for   ",samp1,"   ",samp2,"   (",line,"/",length(combos[,1]),")\n"))
